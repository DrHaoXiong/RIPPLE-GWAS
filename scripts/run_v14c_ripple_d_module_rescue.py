#!/usr/bin/env python
"""Run RIPPLE-D V1.4c locus-aware distributed module repair diagnostics."""
...
DEFAULT_OUT_DIR = ANALYSIS_ROOT / "tier4_v14c_locus_distributed_module_repair_v0_1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--disable-annotation-matching", action="store_true")
...
def render_report(trait: str, modules: pd.DataFrame, summary: dict[str, object]) -> str:
    lines = [
        f"# RIPPLE-D V1.4c Module Repair Diagnostics: {trait}",
        "",
        ...
        f"- Module-specific-rank supported modules: {summary.get('n_module_specific_rank_supported_module', 0):,}",
        ...
        "## Top Rows",
        "",
        "| Rank | Module | Status | n genes | n loci | RIPPLE-D | locus P | positive P | module-rank P | leave-top1 P | n eff loci | top1 locus |",
        "|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for row in tested.head(20).itertuples(index=False):
        lines.append(
            ...
            f"{float(row.positive_locus_empirical_p):.4g} | {float(row.module_specific_rank_empirical_p):.4g} | "
    ...
    lines.extend(
        [
            ...
            "- `mixed_sparse_distributed_candidate`, `moderate_locus_supported_module`, and `module_specific_rank_supported_module` are diagnostic evidence tiers, not final manuscript module claims by themselves.",
...
def main() -> None:
    ...
    for trait_idx, trait in enumerate(args.traits):
        ...
        for window_idx, locus_window_bp in enumerate(locus_windows):
            config = RippleDConfig(
                ...
                annotation_matching_enabled=not args.disable_annotation_matching,
            ...
            prefix = f"{trait}.w{int(locus_window_bp)}bp.v14c"
            summary_path = reports_dir / f"{prefix}_ripple_d_summary.json"
            if args.resume and output_complete(tables_dir, reports_dir, prefix, write_null_details=args.write_null_details):
                ...
                continue
            print(
                f"Running RIPPLE-D V1.4c for {trait}, locus_window={int(locus_window_bp):,} bp, "
    ...
    write_table(tables_dir / "v14c_ripple_d_cross_trait_window_summary.tsv", summary_table)
    print(f"Wrote RIPPLE-D V1.4c diagnostics to {args.out_dir}", flush=True)
